...
def first_container(
    values: list[Any], pattern: re.Pattern[str] = india.IN_PATTERNS.container_number
) -> str | None:
    for value in values:
...
def first_match(values: list[Any], pattern: re.Pattern) -> Any:
    for value in values:
...
def normalize_date(
    value: Any,
    pattern: re.Pattern[str] = india.IN_PATTERNS.date,
...
def normalize_time(
    value: Any,
    pattern: re.Pattern[str] = india.IN_PATTERNS.time,
...
def extract_records(workbook_path: Path) -> list[Record]:
    ...
    try:
        for sheet in workbook.worksheets:
            ...
            remark_col = find_remark_column(
                sheet, merge_map, fallbacks.in_remark_markers
            )
            for row in range(1, sheet.max_row + 1):
                ...
                records.append(Record(
                    ...
                    date=normalize_date(
                        first_match(raw_values, patterns.date),
                        patterns.date,
                        fallbacks.date_formats,
                        fallbacks.whitespace_pattern,
                    ),
                    time=normalize_time(
                        first_match(raw_values, patterns.time),
                        patterns.time,
