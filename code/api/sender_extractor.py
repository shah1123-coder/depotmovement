...
def extract_original_sender_domain(json_response):
    ...
    # The very first email address in the body_preview is the original sender
    # (e.g. "From: GOLDENHORN CONTAINERS <opstuticorin@goldenhorncontainers.com> ...").
    all_emails = re.findall(email_pattern, clean_text)
    sender_email = all_emails[0] if all_emails else "Not Found"

    # Strip everything before the @ sign, keep the domain with the @
